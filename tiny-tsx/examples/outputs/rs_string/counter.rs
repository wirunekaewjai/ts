...
pub fn counter(count: i32) -> String {
    return format!(
        r#"<div class="p-2 flex flex-row items-center" hx-target="this" hx-swap="outerHTML"><button class="w-8 h-8 bg-red-600 text-white rounded-md shadow-md" hx-get="/@counter" hx-vals="{}">+</button></div>"#,
        escape!(format!("{{\"count\"\"count - 1\"{}}}", count - 1))
    );
}
